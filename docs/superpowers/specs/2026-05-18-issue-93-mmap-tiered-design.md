...
**Status:** Design recorded (rev4) — **supersedes the recommendation half of [#187](https://github.com/YurtOS/yurtos-kernel/pull/187)** (`2026-05-17-…`, header amended to defer here). The **tiered *decision*** (§9-A) is ready for maintainer ratification now; the **Tier-A *program*** (§9-B) is a separate, larger ratification. No code in this PR (#93 = decision first; brainstorming gate forbids implementation before an approved spec). Part of #83; refs #52, #71, #65, #97, #129/#168/#172, #218.
...
**Honest cost statement (B1).** The cross-instance memory-copy primitive Tier B needs — `kh_process_mem_write` (and `_read`, `_resume`) — is **declared but unimplemented**: `packages/kernel-wasm/src/kh.rs:1059/1065/1073` are `#[allow(dead_code)]`, and the host side in `packages/runtime-wasmtime/src/kernel_host_interface.rs:4685/4700/4715` is *policy-check → `-EACCES`* then **`-ENOSYS`** (no copy; absent entirely in non-wasmtime runtimes). Read-style syscalls therefore do **not** deliver bytes via this primitive today. The ratifiable floor (slices 1–2) is: **implement the staged-but-`ENOSYS` cross-instance copy end-to-end *and* build mmap on it** — *not* "reuse the read path." This is more than a consume-an-existing-primitive task; the slice-1 risk and timeline reflect that.
...
**Arena ↔ heap isolation is NET-NEW Rust design.** The Rust `yurt-kernel-wasm`/`runtime-wasmtime` path has **no** `brk`/`sbrk`/arena ceiling today (grep-confirmed). The arena is a fixed high window `[__yurt_mmap_arena_base, __yurt_mmap_arena_end)` above the libc heap ceiling at link time, with the guest allocator's `brk`/`sbrk` ceiling pinned strictly below it.
...
- **Tier B (browser / guest-owned memory):** only the **guest** can `memory.grow` its own memory. A safe-Rust guest-libc helper (invoked by, not logic in, the thin C shim) `memory.grow`s to cover `[base,end)` **at process init** (eager to the policy-bounded reserved ceiling). **Ordering invariant:** the arena window is guest-grown *before* any mapping is placed, so the kernel's populate write targets already-addressable memory. No new kernel→host growth import.
...
**`addr`/`MAP_FIXED`.** Non-FIXED → hint ignored, kernel returns an arena address. `MAP_FIXED` outside the arena → returns **`EINVAL`** *and* an enumerated-`UNSUPPORTED` conformance row (the tag is the classification, not a separate return path). `MAP_FIXED` inside the arena at a free range **is** honored.
...
**`PROT_*` is NOT enforced — bookkeeping-only on BOTH tiers (P0b/M1).** Tier A maps host pages RW regardless of guest `prot`: a guest *store* to a `PROT_READ` page, or any access to `PROT_NONE`, would raise a **native SIGSEGV in the embedder**, not a guest-visible fault — no portable wasm host-fault→guest-fault delivery. Applies to the initial mapping `prot`, not only `mprotect`. `prot` is recorded (for `/proc`) and **never enforced** either tier; enforcement enumerated-`UNSUPPORTED`. A real host-fault→guest-trap mechanism is a separate future design. Therefore Tier A is **"lazy + COW + writable-fd `MAP_SHARED`-coherency correct; `PROT_*` not enforced"** — *not* "full MMU-correct". That is the honest claim.

**EOF — hybrid, no silent coherency loss (D).** A host file mapping accessed past the file's last page would `SIGBUS` the *embedder*. When `offset+len > round_up(file_size,page)`, Tier A maps the **integral in-file region via Tier A** (preserving `MAP_SHARED` coherency for the part that has it) and **zero-fills only the partial tail page(s) within the *same* mapping** (anon overlay) — it does **not** downgrade the whole map to Tier B (that would silently drop the coherency Tier A exists to provide — the P1b silent-wrong class). True POSIX `SIGBUS`-past-EOF and the tail-page zero-fill are enumerated-`UNSUPPORTED` rows (no guest-fault delivery).

**Scale, honestly (M2).** Lazy, no eager-copy/double-buffer — far better than Tier B for large files — but the arena lives in the user instance's single linear memory: on `wasm32` the ceiling is the ~4 GiB budget *shared with heap+stack*. `memory64` (where available; orthogonal to the §1.1 aliasing rejection) is the only lever past 4 GiB.
...
- **`MAP_SHARED,fd`** — **shadow-diff writeback** (fixes #187's silent-corruption footgun). Shadow at map time; on `msync(MS_SYNC)`/`munmap`, diff current-vs-shadow, `pwrite` **only changed ranges**, refresh shadow → concurrent external writers not clobbered. **Read coherency also lost (M3):** map-time snapshot for reads too — an external writer's updates are never observed; enumerated-`UNSUPPORTED`. Not destructive, not coherent. Cost 2× region + O(len) diff; same cap.
...
3. **Abnormal-death teardown is embedder-owned RAII, not kernel-driven (E).** `MmapTable` lives in `kernel.wasm`, but Tier-A host VA + host fd live in the embedder. On kill/panic/abnormal termination the guest never calls `munmap` and the kernel teardown may not run. The embedder therefore binds every Tier-A host mapping/fd to the user instance's lifetime and releases them in `kh_destroy_instance` (host side exists: `packages/runtime-wasmtime/src/kernel_host_interface.rs`), as RAII on instance drop — so a killed process **cannot leak host VA or an fd** even if `kernel.wasm` never runs cleanup. (Tier B has no host mapping → nothing to leak.)
...
| `EINVAL` | `len==0`; bad `prot`/`flags` (incl. `MAP_PRIVATE`+`MAP_SHARED`); non-page-aligned `offset`; `MAP_FIXED` outside arena; `addr/len/offset` overflow (Linux uses EINVAL, not EOVERFLOW) |
| `ENODEV` | OFD type unmappable (socket/pipe/char-special) — Linux/FUSE-direct_io precedent |
| `ENOMEM` | arena exhausted; Tier-B `len > YURT_MMAP_MAX_COPY` (default 64 MiB) |
...
**Consumer integration proofs:** sqlite-mmap B0 zero-diff (both tiers; Tier B via the cooperative VFS — also proves the windowing component and the bundled-VFS scoping of C); Polars/Arrow small correct both tiers + **large** asserts Tier-B `ENOMEM`→identical non-mmap-fallback output, Tier A correct + memory bounded; Python `mmap` **concurrent-writer regression** (external writer touches X; guest maps, writes Y elsewhere, `msync`; assert X intact) — proves shadow-diff; **EOF test** (map past EOF: Tier-B zero-fill no-crash; Tier-A integral region + tail zero-fill, `MAP_SHARED` coherency on the integral part preserved); **RO-fd `MAP_SHARED` read test** (must succeed, not `EACCES`); **abnormal-death test** (kill a process holding a Tier-A map; assert embedder RAII released host VA+fd); anon-shared two-pthread (real under SAB, `ENOTSUP` otherwise).
...
1. `SYS_*` ABI + `MmapSelector` + arena allocator + **net-new Rust arena↔heap isolation + guest-side arena growth** + errno/overflow + wasm32-width tests; **private-anon** (arena/zero-fill); **implement `kh_process_mem_write` end-to-end (host side in `runtime-wasmtime`, currently `-ENOSYS`)**; **Tier-B `MAP_PRIVATE`** on it; makes `mmap/munmap/mprotect/msync` functional (`madvise` unchanged).
...
4. **New `kh_mmap_*` import** + Tier A host `MAP_FIXED` overlay in `runtime-wasmtime` + per-runtime user-instance memory ownership (wasmtime first) + fd-mode flag derivation + host-RW/prot-not-enforced + **embedder RAII teardown via `kh_destroy_instance`** + EOF hybrid.
...
- [x] Recommendation recorded (rev4): Tier B universal floor + Tier A native zero-copy + A′ deferred to #218.
