...
- **Tier B (browser / guest-owned memory):** only the **guest** can `memory.grow` its own memory. **Two-phase reserve-then-grow (High-2b — eager-grow would OOM non-mmap workloads):** at process init the arena is only a **VA reservation** of `[base,end)` (address-space bookkeeping; **zero accessible cost** — a process that never calls `mmap` pays nothing). Accessible size is grown **lazily, per mapping, on demand**: `sys_mmap` is **two-phase** — the kernel selects the arena range and returns `NEED_GROW(range)`; the safe-Rust guest-libc helper (invoked by, not logic in, the thin C shim) `memory.grow`s exactly that range; the shim re-enters and the kernel then populates via `kh_process_mem_write`. **Ordering invariant:** the specific mapping's range is guest-grown *before* the kernel populates it. Total accessible arena is bounded by a **per-process cap `YURT_MMAP_ARENA_MAX` (default 256 MiB; `PolicyEnforcer` knob)**, distinct from the per-mapping `YURT_MMAP_MAX_COPY` (§5); exceeding it → `ENOMEM`. No new kernel→host growth import.
...
1. `SYS_*` ABI + `MmapSelector` + arena allocator + **net-new Rust arena↔heap isolation + two-phase reserve-then-grow (per-mapping, `YURT_MMAP_ARENA_MAX`-bounded)** + errno/overflow + wasm32-width tests; **private-anon** (arena/zero-fill); **implement `kh_process_mem_write` end-to-end (wasmtime host side, currently `-ENOSYS`) AND add the missing `mayProcessMemory` hook to the JS `PolicyEnforcer` (F1 security-parity)**; **Tier-B `MAP_PRIVATE`** on it; makes `mmap/munmap/mprotect/msync` functional (`madvise` unchanged).
