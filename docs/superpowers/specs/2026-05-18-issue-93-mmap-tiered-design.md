...
**Status:** Design recorded (rev6) — **supersedes the recommendation half of [#187](https://github.com/YurtOS/yurtos-kernel/pull/187)** (`2026-05-17-…`, header amended to defer here). The **tiered *decision*** (§9-A) is ready for maintainer ratification now; the **Tier-A *program*** (§9-B) is a separate, larger ratification. No code in this PR (#93 = decision first; brainstorming gate forbids implementation before an approved spec). Part of #83; refs #52, #71, #65, #97, #129/#168/#172, #218.
...
- **Tier B (browser / guest-owned memory):** only the **guest** can `memory.grow` its own memory. **Two-phase reserve-then-grow (High-2b):** at process init the arena is only a **VA reservation** of `[base,end)` (bookkeeping; zero accessible cost). Accessible memory is grown **lazily** to the new arena high-water mark: `sys_mmap` is **two-phase** — the kernel selects the arena range and returns `NEED_GROW(new_high_water)`; the safe-Rust guest-libc helper `memory.grow`s linear memory to that mark; the shim re-enters and the kernel populates via `kh_process_mem_write`. **Ordering invariant:** the mapping's range is guest-grown *before* the kernel populates it.

  *Robustness (Issue-3):*
  - **C-shim purity** ([[feedback_buffer_code_in_rust]]): C `mmap` is literally `return __yurt_mmap(addr,len,prot,flags,fd,offset);` — the entire phase1→`NEED_GROW`→`memory.grow`→re-enter loop lives in the **safe-Rust guest helper**; no branch logic in C.
  - **Grow-failure:** if phase-2 `memory.grow` fails, the helper re-enters with a failure token; the kernel **rolls back the phase-1 arena reservation** and `sys_mmap` returns `ENOMEM` — no partial/leaked reservation.
  - **Concurrency:** the arena allocator + `MmapTable` are per-process kernel state mutated under the existing serialized kernel dispatch ([[project_async_dispatcher_pass]]); concurrent `mmap` from multiple pthreads on shared memory serialize there, and `memory.grow` is monotonic so racing grows converge (grow-to-mark is idempotent).
  - **Async composition:** if the populate read is over an async VFS (S3/network), the AsyncBridge suspend occurs **after** phase-2 grow + re-enter, *during* the kernel's VFS read (before `kh_process_mem_write`) — never interleaved with the grow handshake. Order: phase1 reserve → `NEED_GROW` → guest grow → re-enter → [VFS read, may AsyncBridge-suspend] → `process_mem_write` → return.
...
- [x] Recommendation recorded (rev6): Tier B universal floor + Tier A native zero-copy + A′ deferred to #218.
