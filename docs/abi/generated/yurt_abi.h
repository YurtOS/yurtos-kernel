...
/* Connect a socket fd to a POSIX sockaddr record. Flags may include YURT_SOCKET_FLAG_TLS. */
__attribute__((import_module("yurt"), import_name("host_socket_connect")))
int yurt_host_socket_connect(int fd, int addr_ptr, int addr_len, uint32_t flags);

/* Bind a socket fd to a POSIX sockaddr record. */
...
__attribute__((import_module("yurt"), import_name("host_idb_get")))
int yurt_host_idb_get(int req_ptr, int req_len, int out_ptr, int out_cap);

/* Write a durable KV entry. Request is u8 store_len + store + u32 key_len LE + key + value. */
__attribute__((import_module("yurt"), import_name("host_idb_put")))
int yurt_host_idb_put(int req_ptr, int req_len);

/* Remove a durable KV entry. Request is u8 store_len + store + key bytes. */
__attribute__((import_module("yurt"), import_name("host_idb_delete")))
int yurt_host_idb_delete(int req_ptr, int req_len);

/* List keys in a store. Request is u8 store_len + store + prefix bytes (empty = all). Writes count + (len, bytes)*. */
__attribute__((import_module("yurt"), import_name("host_idb_list")))
int yurt_host_idb_list(int req_ptr, int req_len, int out_ptr, int out_cap);
