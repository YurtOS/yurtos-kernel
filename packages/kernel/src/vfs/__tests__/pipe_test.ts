...
describe("AsyncPipe", () => {
  ...
  it("waitWritable resolves when a read frees capacity", async () => {
    const [read, write] = createAsyncPipe(4);
    write.write(new Uint8Array(4).fill(0x41));

    let resolved = false;
    const writable = write.waitWritable().then(() => {
      resolved = true;
    });
