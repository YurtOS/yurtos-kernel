...
export function createTtyState(ttyId: number): TtyState {
  return {
    ttyId,
    toSlave: [], toSlaveWaiters: [],
    toMaster: [], toMasterWaiters: [],
    fgPgid: 0, cols: 80, rows: 24, masterClosed: false,
  };
}

export function createTtyMasterTarget(state: TtyState): FdTarget & { type: 'tty_master' } {
  return { type: 'tty_master', ttyId: state.ttyId, state };
}

export function createTtySlaveTarget(state: TtyState): FdTarget & { type: 'tty_slave' } {
...
export class TtyHandle {
  ...
  get rows(): number { return this.state.rows; }
  get cols(): number { return this.state.cols; }
  get fgPgid(): number { return this.state.fgPgid; }
}
