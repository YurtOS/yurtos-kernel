...
describe(
  ...
  () => {
    ...
    fileIt("reports its version", async () => {
    ...
    fileIt("classifies ASCII text", async () => {
      sandbox = await Sandbox.create({
        wasmDir: WASM_DIR,
        adapter: new NodeAdapter(),
      });
    ...
    fileIt("classifies an empty file", async () => {
      sandbox = await Sandbox.create({
        wasmDir: WASM_DIR,
        adapter: new NodeAdapter(),
      });
    ...
    fileIt("classifies a WebAssembly binary", async () => {
      sandbox = await Sandbox.create({
        wasmDir: WASM_DIR,
        adapter: new NodeAdapter(),
      });
    ...
    fileIt("classifies JSON via the magic database", async () => {
      sandbox = await Sandbox.create({
        wasmDir: WASM_DIR,
        adapter: new NodeAdapter(),
      });
      sandbox.writeFile(
        "/tmp/data.json",
        new TextEncoder().encode('{"a":1,"b":[2,3]}\n'),
      );
