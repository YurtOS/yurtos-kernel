...
function buildBase(id: string): MemoryRoot {
  ...
  base.addFile('/home/user/keep.txt', 'base-keep', {
    uid: 1000, gid: 1000, permissions: 0o644,
  });
...
describe('Persistence serializer — OverlayVFS round-trip', () => {
  it('preserves whiteouts and upper writes across export/import', () => {
    const vfs = new OverlayVFS({ base: buildBase('base:v1'), upper: new VFS() });
    ...
    const restored = new OverlayVFS({ base: buildBase('base:v1'), upper: new VFS() });
    importState(restored, blob);

    expect(dec.decode(restored.readFile('/home/user/keep.txt'))).toBe('base-keep');
    expect(() => restored.readFile('/home/user/drop.txt')).toThrow(/ENOENT/);
    expect(dec.decode(restored.readFile('/home/user/new.txt'))).toBe('upper-new');
  });
  ...
  it('rejects an import whose baseId differs from the target overlay', () => {
    ...
    const target = new OverlayVFS({ base: buildBase('base:v2'), upper: new VFS() });
  ...
  it('non-overlay blob (includeBase) imports cleanly into a plain VFS', () => {
    const source = new OverlayVFS({ base: buildBase('base:v1'), upper: new VFS() });
