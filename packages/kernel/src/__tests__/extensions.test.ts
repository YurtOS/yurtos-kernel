 ...
 * piped stdin, --help, which discovery, and Python package VFS.
 */
  ...
  it("multiple extensions in one sandbox", async () => {
    sandbox = await Sandbox.create({
      ...
      extensions: [
        {
          name: "ext1",
          command: async () => ({ stdout: "one\n", exitCode: 0 }),
        },
        {
          name: "ext2",
          command: async () => ({ stdout: "two\n", exitCode: 0 }),
        },
      ],
    });
...
describe("Extension Python packages", () => {
      ...
      extensions: [{
        ...
        pythonPackage: {
          ...
          files: {
            ...
            "utils.py": 'def hello(): return "hi"',
          },
        },
      }],
    });
    const init = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/testpkg/__init__.py"),
    );
    expect(init).toBe("# test package");
    const utils = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/testpkg/utils.py"),
    );
    expect(utils).toContain("def hello");
  });

  it("yurt_ext.py bridge is installed when packages exist", async () => {
    sandbox = await Sandbox.create({
      wasmDir: WASM_DIR,
      adapter: new NodeAdapter(),
      extensions: [{
    ...
    const content = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/yurt_ext.py"),
    );
  ...
  it("command-only extension gets auto-generated _shim.py and __init__.py", async () => {
    sandbox = await Sandbox.create({
      ...
      extensions: [{
        name: "greet",
        command: async (input) => ({
          stdout: `hello ${input.args[0]}\n`,
          exitCode: 0,
        }),
      }],
    });
    const shim = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/greet/_shim.py"),
    );
    expect(shim).toContain("import yurt_ext");
    expect(shim).toContain("def run(*args");
    expect(shim).toContain('_ce.call("greet"');
    const init = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/greet/__init__.py"),
    );
  ...
  it("extension with command+package gets _shim.py alongside author files", async () => {
    sandbox = await Sandbox.create({
      ...
      extensions: [{
        ...
        pythonPackage: {
          version: "1.0.0",
          files: { "__init__.py": "from myext._shim import run" },
        },
      }],
    });
    const shim = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/myext/_shim.py"),
    );
    expect(shim).toContain('_ce.call("myext"');
    const init = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/myext/__init__.py"),
    );
  ...
  it("extension with both command and package", async () => {
    ...
    // Package files exist
    const init = new TextDecoder().decode(
      sandbox.readFile("/usr/lib/python/dualext/__init__.py"),
    );
