...
async function createShellBaseRoot(): Promise<string> {
  ...
  const bashFixture = typeof WebAssembly.Suspending === 'function'
    ? 'bash.wasm'
    : 'bash-asyncify.wasm';
  await copyFile(join(WASM_DIR, bashFixture), join(baseRoot, 'bin/bash'));
  await copyFile(join(WASM_DIR, 'busybox.wasm'), join(baseRoot, 'usr/bin/busybox'));
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      files: [
        { path: '/bin', type: 'dir', uid: 0, gid: 0, mode: 0o755 },
        { path: '/bin/bash', type: 'file', uid: 0, gid: 0, mode: 0o755 },
        { path: '/etc', type: 'dir', uid: 0, gid: 0, mode: 0o755 },
        { path: '/etc/base-marker.txt', type: 'file', uid: 1000, gid: 1000, mode: 0o644 },
        { path: '/etc/yurt', type: 'dir', uid: 0, gid: 0, mode: 0o755 },
        { path: '/etc/yurt/base-image.json', type: 'file', uid: 0, gid: 0, mode: 0o644 },
        ...
        { path: '/usr/bin/mkdir', type: 'symlink', uid: 0, gid: 0, mode: 0o777 },
...
describe('Sandbox baseRoot', { sanitizeResources: false, sanitizeOps: false }, () => {
  it('boots from a read-only base root and writes changes only to the upper layer', async () => {
    ...
    try {
      ...
      expect(() => sandbox.writeFile('/etc/yurt/base-image.json', enc.encode('{}'))).toThrow(/EACCES/);
      expect(() => sandbox.mkdir('/etc/backdoor.d')).toThrow(/EACCES/);
      expect(dec.decode(sandbox.readFile('/etc/yurt/base-image.json'))).toContain('test-base-root');
    } finally {
      sandbox.destroy();
    }
  });
  ...
  it('rejects guest attempts to shadow root-owned base files', async () => {
    ...
    const sandbox = await Sandbox.create({
      ...
      baseRoot,
      bootArgv: ['/bin/bash'],
      bootWasmPath: join(WASM_DIR, 'bash.wasm'),
    });
    ...
    try {
      ...
      const etcWrite = await sandbox.run('echo evil > /etc/yurt/base-image.json');
      ...
      expect(dec.decode(sandbox.readFile('/etc/yurt/base-image.json'))).toContain('test-shell-base-root');
      expect(() => sandbox.readFile('/etc/backdoor.d/payload')).toThrow();
      expect(dec.decode(await readFile(join(baseRoot, 'etc/yurt/base-image.json')))).toContain('test-shell-base-root');
    } finally {
      sandbox.destroy();
    }
  });
  ...
  it('rejects guest renames into and out of root-owned base directories', async () => {
    ...
    try {
      const intoEtc = await sandbox.run('echo payload > /tmp/payload; mv /tmp/payload /etc/payload');
      expect(intoEtc.exitCode).not.toBe(0);
      expect(dec.decode(sandbox.readFile('/tmp/payload')).trim()).toBe('payload');
      expect(() => sandbox.readFile('/etc/payload')).toThrow();

      const outOfBin = await sandbox.run('mv /bin/bash /tmp/bash');
      expect(outOfBin.exitCode).not.toBe(0);
      expect(sandbox.stat('/bin/bash').type).toBe('file');
      expect(() => sandbox.readFile('/tmp/bash')).toThrow();
    } finally {
      sandbox.destroy();
    }
  });
