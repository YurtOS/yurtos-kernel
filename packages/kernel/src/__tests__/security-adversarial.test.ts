...
describe('Security: adversarial inputs', () => {
  ...
  it('chmod on system config file is denied', async () => {
    ...
    const result = await sb.run('chmod 777 /etc/yurt/pkg-policy.json');
    expect(result.exitCode).not.toBe(0);
    sb.destroy();
  });
  ...
  it('write to read-only system file is denied', async () => {
    ...
    // /etc/yurt/pkg-policy.json is 0o444 — writes are denied
    const result = await sb.run('echo evil > /etc/yurt/pkg-policy.json');
    expect(result.exitCode).not.toBe(0);
    sb.destroy();
  });
  ...
  it('importState cannot overwrite system config files', async () => {
    ...
    // Tamper with the blob: inject an entry targeting /etc/yurt/pkg-policy.json
    const jsonBytes = blob.subarray(12);
    const state = JSON.parse(new TextDecoder().decode(jsonBytes));
    state.files.push({
      path: '/etc/yurt/pkg-policy.json',
    ...
    try {
      ...
      const originalConfig = new TextDecoder().decode(sb2.readFile('/etc/yurt/pkg-policy.json'));

      sb2.importState(tamperedBlob);

      // Config must be unchanged — the tampered entry was silently skipped
      const afterConfig = new TextDecoder().decode(sb2.readFile('/etc/yurt/pkg-policy.json'));
