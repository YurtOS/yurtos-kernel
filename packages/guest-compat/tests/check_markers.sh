...
for pair in "${pairs[@]}"; do
  ...
  defined_marker="$("$NM" --defined-only "$path" | awk -v s="__yurt_guest_compat_marker_$sym" '$3 == s {print $3; exit}')"
  if [ -z "$defined_sym" ]; then
    echo "FAIL: $sym not defined in $obj" >&2; fail=1
  fi
  if [ -z "$defined_marker" ]; then
    echo "FAIL: __yurt_guest_compat_marker_$sym not defined in $obj" >&2; fail=1
  fi
done
