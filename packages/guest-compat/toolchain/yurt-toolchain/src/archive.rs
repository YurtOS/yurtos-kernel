...
/// Assert the `yurt_guest_compat_version` sentinel symbol is defined in
...
pub fn check_version(nm: &Path, archive: &Path) -> Result<()> {
    ...
    let present = stdout.lines().any(|line| {
        line.split_whitespace()
            .last()
            .map(|s| s == "yurt_guest_compat_version")
            .unwrap_or(false)
    });
    if !present {
        return Err(anyhow!(
            "archive {} does not define yurt_guest_compat_version (§Versioning)",
            archive.display()
        ));
    }
