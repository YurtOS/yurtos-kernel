...
HDR="$REPO_ROOT/abi/include/yurt_abi.h"
...
# The symbol must be defined (D or R) and named `yurt_abi_version`.
if ! "$NM" --defined-only "$OBJ" | grep -E ' [DR] yurt_abi_version$' >/dev/null; then
  echo "yurt_abi_version not defined in $OBJ" >&2
  "$NM" --defined-only "$OBJ" >&2
  exit 1
fi

# The header must expose YURT_ABI_VERSION_MAJOR and _MINOR.
grep -q 'YURT_ABI_VERSION_MAJOR' "$HDR" || { echo "missing major in header" >&2; exit 1; }
grep -q 'YURT_ABI_VERSION_MINOR' "$HDR" || { echo "missing minor in header" >&2; exit 1; }
