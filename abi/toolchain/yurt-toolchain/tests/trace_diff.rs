use yurt_toolchain::spec::{Case, Expected};
use yurt_toolchain::trace::{diff_case, parse_trace_line, Mismatch, TraceLine};
...
fn diff_reports_process_vs_trace_exit_disagreement() {
    ...
    assert!(mm
        .iter()
        .any(|m| matches!(m, Mismatch::ProcessTraceExitDisagree { .. })));
}
